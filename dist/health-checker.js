...
class GatewayHealthChecker {
    ...
    async checkSessions() {
        try {
            const sessionsDir = this.config.get('openclaw.sessions_dir') ||
                path.join(this.config.get('openclaw.dir'), 'sessions');
    ...
    async checkQueue() {
        try {
            ...
            const maxQueueAge = this.config.get('health_check.max_queue_age_hours', 2);
    ...
    async checkProviders() {
        try {
            ...
            const threshold = this.config.get('health_check.provider_error_threshold', 10);
